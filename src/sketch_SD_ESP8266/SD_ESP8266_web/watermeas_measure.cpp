...
int measure_start_run(SdClassWrap* sd, SD_dbconf* dbconf, char* write_buffer){
    ...
    if(measurement_run_initialized){
        write_buffer_count += sprintf(&write_buffer[write_buffer_count], "Measurement run already active.\n");
    ...
    if(measure_file){
        ...
        write_buffer_count += sprintf(&write_buffer[write_buffer_count], "Measurement started using the file name: %s\n", file_name);
    }
    else{
        measurement_run_active = LOW;
        measurement_run_initialized = LOW;
        write_buffer_count += sprintf(&write_buffer[write_buffer_count], "The measurement run could not be started (check used file name).\n");
    }
...
int measure_stop_run(SdClassWrap* sd, SD_dbconf* dbconf, char* write_buffer){
    uint32_t write_buffer_count = 0;
    SD_endRun(&measure_file);
    measurement_run_active = LOW;
    measurement_run_initialized = LOW;
    write_buffer_count += sprintf(&write_buffer[write_buffer_count], "Ongoing measurement run stopped.\n");
    return measurement_run_active;
}

//Manage measure run start and end.
int measure_control(SdClassWrap* sd, SD_dbconf* dbconf, char* write_buffer){
    ...
    if(measure_button_pressed){
        ...
        if(measure_button_first_time){
            ...
            if(measurement_run_active)
                measure_start_run(sd, dbconf, write_buffer); //Measurement run can be set to low here if initialization failed.
            else
                measure_stop_run(sd, dbconf, write_buffer);
        }
